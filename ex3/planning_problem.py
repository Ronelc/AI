    ...
    def get_successors(self, state):
        ...
        for act in action:
            if act.all_preconds_in_list(state) and not act.is_noop():
                new_pre = frozenset(set(act.get_add()).union(state).difference(act.get_delete()))
                successors.append((new_pre, act, step_cost))
        return successors
...
def max_level(state, planning_problem):
    ...
    g = [pg_init]
    while not planning_problem.is_goal_state(stat_temp):
        if is_fixed(g, i):
            return float('inf')
        pre_lyer = pg_init
        pg_init = PlanGraphLevel()
        g.append(pg_init)
        pg_init.expand_without_mutex(pre_lyer)
        stat_temp = pg_init.get_proposition_layer().get_propositions()
        i += 1
    return i
...
def level_sum(state, planning_problem):
    ...
    while not planning_problem.is_goal_state(stat_temp):
        ...
        stat_temp = pg_init.get_proposition_layer().get_propositions()
        for p in stat_temp:
            if p in planning_problem.goal and p not in find_gols:
                find_gols.add(p)
                sum_level += i
    return sum_level
